...
files.forEach(file => {
    ...
    if (content.includes('CategoryLayout')) {
        ...
        description = `The main landing/home page of the application, showcasing hero sections, top categories, and curated experiences.`;
    } else if (name.includes('Auth') || name === 'SignIn' || name === 'SignUp') {
        description = `Authentication related page for user login, registration, or callback handling.`;
    } else if (name === 'Cart') {
        description = `Displays the user's shopping cart and allows modification before checkout.`;
    } else if (name === 'Dashboard') {
        description = `User profile and dashboard for managing bookings, wishlists, and account details.`;
    } else if (name === 'SearchResults') {
        description = `Displays dynamic search results based on user queries, utilizing Supabase Edge Functions.`;
    }
    ...
    if (content.includes('@/data/catalog')) dataDeps.add('catalog.ts (Product Catalog)');
    if (content.includes('supabase')) dataDeps.add('Supabase API');
    if (content.includes('fetch(') || content.includes('axios')) dataDeps.add('External API / REST');
